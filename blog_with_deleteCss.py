...
# Bu fayl sənin başdan sona blog saytı yaratmağın üçün izah edilmiş versiyasıdır
...
<body>
    ...
    {% for post in posts %}
        <div class="post">
            <h2>{{ post.title }}</h2>
            <p>{{ post.content }}</p>
            <a href="{% url 'edit_post' post.pk %}" class="button">Edit</a>
            <a href="{% url 'delete_post' post.pk %}" class="button" onclick="return confirm('Silmek istediyinize eminsiniz?')">Delete</a>
        </div>
    {% endfor %}
    </div>
</body>
...
# Hazırdır! Add, Edit və Delete əməliyyatları tam işləyər.
